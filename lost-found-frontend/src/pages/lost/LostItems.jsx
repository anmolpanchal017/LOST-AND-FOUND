...
import { MapPin, Phone, Calendar, Tag, PackageSearch, Loader2, ArrowRight } from "lucide-react";
      ...
      <main className="flex-grow container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-12">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-orange-600 font-bold text-sm uppercase tracking-wider">Help Community</span>
            </div>
            <h1 className="text-4xl font-bold font-serif text-slate-900 tracking-tight">Lost Items</h1>
            <p className="text-slate-500 mt-2">Help reunite owners with their lost belongings.</p>
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-400">
            ...
            <p className="text-lg font-medium">Scanning for lost items...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-24 px-4 text-center bg-white rounded-3xl border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 max-w-md mt-2">
              {searchTerm 
                ? `We couldn't find any lost items matching "${searchTerm}". Try a different term.`
                : "Great news! No lost items currently listed. Check back later."}
            </p>
          </div>
        ) : (
            ...
            {filteredItems.map((item) => (
              ...
              >
                ...
                <div className="relative h-48 overflow-hidden bg-slate-100 italic flex items-center justify-center text-slate-400 text-sm p-4 text-center">
                  ...
                  <div className="absolute top-3 left-3 px-3 py-1 bg-white/90 backdrop-blur shadow-sm rounded-full text-[10px] font-bold text-orange-600 uppercase tracking-widest ring-1 ring-orange-500/10">
                    {item.category}
                  </div>
                </div>
                ...
                <div className="p-5 flex flex-col flex-grow">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      className="flex-grow flex items-center justify-center gap-2 py-3 bg-primary text-white text-xs font-bold rounded-xl hover:bg-primary-hover active:scale-[0.98] transition-all"
