...
import { Upload, Phone, MapPin, Calendar, Tag, FileText, Send, Loader2 } from "lucide-react";
...
export default function LostForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setImage(null);
      setPreview(null);
      if (document.getElementById("lost-image-input")) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <div className="relative group">
          ...
          <textarea
            name="description"
            placeholder="Add specific details (brand, color, markings...)"
            rows="2"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1">
          <label className="text-xs font-bold text-slate-500 uppercase ml-1">Date Lost</label>
          <div className="relative group">
      ...
      <button 
        type="submit" 
        disabled={loading}
        className="w-full py-3 bg-primary text-white font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-primary-hover active:scale-[0.98] transition-all disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-2"
      >
        {loading ? (
